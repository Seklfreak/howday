...
/// across actors — which is the whole point of decoding it in
/// `ContactDirectory` rather than in the card's body.
///
/// Equatable by identity: the directory caches and re-hands the same decoded
/// instance, so this is what lets SwiftUI skip a card whose photo hasn't
/// changed instead of redrawing the whole grid on every reload.
struct Avatar: @unchecked Sendable, Equatable {
    let image: UIImage

    static func == (lhs: Avatar, rhs: Avatar) -> Bool { lhs.image === rhs.image }
}

struct BoardEntry: Identifiable, Sendable, Equatable {
    let id: UUID
